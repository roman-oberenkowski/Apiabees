...
class EditModal extends Component
{
    ...
    {
        $rules = Apiary::validationRulesCreate();
        unset($rules['code_name']);
        unset($rules['name']);
        $rules['code_name'] = ['required', 'string', 'min:2', 'max:32', Rule::unique('apiaries', 'code_name')->ignore($this->old_code_name, 'code_name')];
        $rules['name'] = ['required', 'string', 'min:2', 'max:64', Rule::unique('apiaries', 'name')->ignore($this->old_code_name, 'code_name')];
        return $rules;
    }
